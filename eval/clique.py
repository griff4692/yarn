...
def decorate_transform(source_input, clique_by_type, max_toks=8192):
    ...
    if args.filter_clique_notes or clique_source_toks > max_toks or curr_clique['is_retry']:
...
def break_up_big_cliques(cluster_idxs, max_clique_size):
    new_cluster_idxs = []
    for idxs in cluster_idxs:
        if len(idxs) <= max_clique_size:
            new_cluster_idxs.append(idxs)
        else:
            num_splits = int(math.ceil(len(idxs) / max_clique_size))
            idxs_breakup = [list(x) for x in np.array_split(idxs, num_splits)]
            new_cluster_idxs += idxs_breakup
    return new_cluster_idxs


def get_inference_cliques(pred_cluster_spans, source_ents, max_clique_size=10):
    ...
    component_idxs = break_up_big_cliques(component_idxs, max_clique_size=max_clique_size)
...
if __name__ == '__main__':
    ...
    for example in tqdm(data):
        ...
        cliques = get_inference_cliques(pred_source_clusters, ent_info['source_ents'], max_clique_size=100)
        queue = []
        ...
        while len(queue) > 0:
            ...
            for c in cp:
                ...
                if len(cpse_embeds) == 0:
                    ...
                    print(f'Repetitive sentence -> {c}. Will not include.')
                elif c in pred_sents:
                    print(f'Sentence already in summary -> {c}. Will not include.')
                else:
            ...
            cluster_size = len(clique_cluster_is_covered)
            clique_cov_frac = sum(clique_cluster_is_covered) / cluster_size
            ...
            if len(uncovered_clique_clusters) > 0:
                if len(covered_clique_clusters) == 0:
                    print('Failed to cover any entities in cluster. Adding them to unused queue.')
